...
///
/// Best to run with `RUST_LOG=debug RUSTFLAGS='-Cdebug-assertions=y' cargo test -r
/// pallet-message-queue stress_test_ -- --ignored`.
///
/// # Example output
///
/// ```pre
...
fn stress_test_queue_suspension() {
	...
	new_test_ext::<Test>().execute_with(|| {
		...
		log::info!("Resumed all {} suspended queues", SuspendedQueues::take().len());
		log::info!("Processing all remaining {} messages", msgs_remaining);
		process_all_messages(msgs_remaining);
		post_conditions();
	});
