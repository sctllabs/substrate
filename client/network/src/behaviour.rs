	...
	Client: HeaderBackend<B> + 'static,
{
	...
	pub fn new(
		...
		let mut mixnet = None;
		if enable_mixnet {
			let libp2p::core::PublicKey::Ed25519(pk) = &local_public_key;
			//TODO: key management
			let mixnet_config = mixnet::Config::new(pk.encode());
			let keystore = Arc::new(mixnet::PublicKeyStore);
			mixnet = Some(MixnetBehaviour::new(mixnet_config, keystore));
		};
		Ok(Self {
			substrate,
...
impl From<mixnet::NetworkEvent> for BehaviourOut {
	fn from(event: mixnet::NetworkEvent) -> Self {
		match event {
