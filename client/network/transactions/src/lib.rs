...
impl TransactionsHandlerPrototype {
	...
	>(
		...
		external_source: TracingUnboundedReceiver<Vec<u8>>,
	...
	S: NetworkPeers + NetworkEventStream + NetworkNotification + sp_consensus::SyncOracle,
{
	...
	pub async fn run(mut self) {
		loop {
			futures::select! {
				...
				transactions = self.external_source.select_next_some() => {
					self.inject_transaction(transactions)
				},
			}
		}
	}
	...
	fn inject_transaction(&mut self, transactions: Vec<u8>) {
		let Ok(transactions) =
		...
		if self.service.is_major_syncing() {
			trace!(target: "sync", "Ignoring transactions while major syncing");
			return
		}

		trace!(target: "sync", "Received {} transactions from the external source", transactions.len());
		for t in transactions {
			...
			if let Entry::Vacant(entry) = self.pending_transactions_peers.entry(hash.clone()) {
